...
class VGA_write_read_seq extends VGA_base_seq;
    `uvm_object_utils(VGA_write_read_seq);
    int num_loop = 0;
    bit [7:0] addr;
    bit [31:0] wdata, rdata;

    function new(string name = "VGA_write_read_seq");
        super.new(name);
    endfunction  //new()

    virtual task body();
        VGA_seq_item item = VGA_seq_item::type_id::create("item");
