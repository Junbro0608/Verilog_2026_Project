...
void SysTick_Run(unsigned int msec) {
    ...
    Macro_Write_Block(SysTick->CTRL, 0x7, 0b000, 0);
    // 주어진 msec 값 만큼의 msec를 count하는 초기값 설정 (LOAD)
    // Macro_Write_Block(SysTick->LOAD, 0xffffffff, msec * (HCLK / (8 * 1000)), 0);
    SysTick->LOAD = msec * (unsigned int)(HCLK / (8 * 1000));
    // VAL 레지스터 값 초기화(0) 및 COUNTFLAG Clear
    Macro_Write_Block(SysTick->VAL, 0xFFFFF, 0x0, 0);
